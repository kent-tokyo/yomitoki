...
> **ステータス: `0.1.0-alpha.1`はcrates.ioに公開済みですが、このリポジトリの現在の内容はそれより先行しています。** 計画中の6コンポーネントすべてが実装済みです(`fragment_rarity`含む)。ただし`fragment_rarity`はopt-inです — yomitoki自体にはfragment-frequencyコーパスが同梱されていないため(AGENTS.md §5.4は巨大バイナリとしてコーパスを直接埋め込むことを禁止しています)、コーパスを構築(`tools/build-fragment-corpus/`)して設定(`AnalysisConfig.fragment_model`)しない限り無効のままです。**設定した場合の注意: `fragment_rarity`のスコアリング式には既知の不具合があります** — 実際のコーパスでend-to-endテストしたところ、対象としていたケース(アスピリンのような一般的な分子)がより難しく判定されてしまい、狙いと逆の結果になることが確認されています。詳細は「制限事項」を参照。公開済みの`0.1.0-alpha.1`はこれらすべてより前のものです — 変更点は[`CHANGELOG.md`](CHANGELOG.md)を参照してください。これはpre-releaseであり、正式な`0.1.0`までに公開APIが変わる可能性があります。現在のスコープと未実装部分については[`docs/architecture.md`](docs/architecture.md)を参照してください。
...
| `fragment_rarity` | 実装済み、opt-in — `AnalysisConfig.fragment_model`にコーパスが設定されていない限り`None`。**スコアリング式に既知の不具合あり、「制限事項」参照** |
...
* `size_topology`のrotatable bond(回転可能結合)に関する項は、市販されている単純な非分岐長鎖分子(回転可能結合は多いが合成難易度はほぼゼロ)を過大評価します — `fragment_rarity`はそうしたフラグメントを一般的/前例のあるものと認識することで補正するはずでしたが、実際の20万分子コーパスでend-to-endテストしたところ、**逆に悪化する**ことが確認されました: dodecaneの`overall.difficulty`はコーパス未設定時の`0.068`から、コーパス設定時には`0.227`まで上昇します。これは定数のチューニング不足ではなく、確認済みの計算式そのものの不具合です — 詳細は`rules::FRAGMENT_RARITY_WEIGHT`のdocコメントを参照。`docs/architecture.md`の "Scoring direction" セクションも参照してください。
