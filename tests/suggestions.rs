...
use rensei::{AnalysisConfig, ExpectedEffect, SuggestionCode};

#[test]
fn bridged_ring_molecule_gets_a_replace_bridged_ring_suggestion() {
    let config = AnalysisConfig::default();
    let report = rensei::analyze_smiles("C1CC2CCC1C2", &config).expect("norbornane");
    assert!(
        report
            .suggestions
            .iter()
...
fn macrocyclic_molecule_gets_a_simplify_macrocycle_suggestion() {
    let config = AnalysisConfig::default();
    let report = rensei::analyze_smiles("C1CCCCCCCC1", &config).expect("9-membered ring");
    assert!(
        report
            .suggestions
            .iter()
...
fn stereo_dense_molecule_gets_a_reduce_density_suggestion() {
    let config = AnalysisConfig::default();
    let report =
        rensei::analyze_smiles("CC(O)C(N)C(C)C(O)C(N)C", &config).expect("stereo-dense fragment");
    assert!(
        report
            .suggestions
            .iter()
...
fn plain_molecule_with_no_matching_findings_gets_no_suggestions() {
    let config = AnalysisConfig::default();
    let report = rensei::analyze_smiles("CCO", &config).expect("ethanol");
...
fn every_suggestion_is_heuristic_never_a_guarantee() {
    ...
    for smiles in ["C1CC2CCC1C2", "C1CCCCCCCC1", "CC(O)C(N)C(C)C(O)C(N)C"] {
        let report = rensei::analyze_smiles(smiles, &config).expect("valid SMILES");
